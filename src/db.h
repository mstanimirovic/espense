...
#define QUERY_EXPENSES "SELECT users.name,categories.name,ROUND(amount,2),description,date FROM expenses JOIN users on user_id = users.id JOIN categories ON category_id = categories.id;"

#define QUERY_INSERT_EXPENSE "INSERT INTO expenses (user_id, category_id, amount, description, date) VALUES (?, ?, ?, ?, ?);"
